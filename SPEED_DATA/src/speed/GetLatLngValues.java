...
//				System.out.println(difference+" "+uptoTime+" "+fromTime);
				for(int minutes = fromMinutes; minutes <= fromMinutes + difference; minutes=minutes+4)
				{
					...
					select = QueryBuilder.select().column("date").column("minutes").column("lat").column("lng").from(prop.getProperty("keyspace"), prop.getProperty("locationtable"))
					...
					for (Row row : results) {
						...
						jObject.put("minutes",String.format("%02d",hourDisplay)+":"+String.format("%02d",minuteDisplay));
			...
			if(filter.equals("year"))
			{
				...
				for(int i = 0; i < dates.size(); i++)
				{
					...
					for (Row row : results) {
						jObject = new JSONObject();
						jObject.put("date",row.getString("date"));
						jObject.put("minutes",String.format("%02d",fromHours)+":"+String.format("%02d",fromMinutes));
						jObject.put("lat",row.getFloat("lat"));
						jObject.put("lng",row.getFloat("lng"));
						jArray.put(jObject);
					}
