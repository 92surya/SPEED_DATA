...
public class GetChartPoints extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// {
			// select =
			// QueryBuilder.select().column("time").column("speed").from(prop.getProperty("keyspace"),
			// prop.getProperty("speedtable"))
			...
			if (filter.equals("min")) {
				select = QueryBuilder.select().column("time").column("speed")
						.from(prop.getProperty("keyspace"), prop.getProperty("speedtable"))
						.where(QueryBuilder.eq("dvb_id", driverid)).and(QueryBuilder.eq("date", fromDate))
						.and(QueryBuilder.gte("time", fromTime)).and(QueryBuilder.lt("time", fromTime + 120));
				results = session.execute(select);
				...
				for (int minutes = fromMinutes; minutes < fromMinutes + 60; minutes++) {
					...
					select = QueryBuilder.select().column("time").raw("max(speed) as max_speed")
							.from(prop.getProperty("keyspace"), prop.getProperty("speedtable"))
					...
					for (Row row : results) {
						jObject = new JSONObject();
						// jObject.put("time",jArray.length());
						jObject.put("datetime",
								start.toString() + " " + hourDisplay + ":" + minuteDisplay + ":" + fromSeconds);
				...
				for (int hours = fromHours; hours < fromHours + 168; hours = hours + 4) {
					...
					for (Row row : results) {
						...
						// jObject.put("time",jArray.length());
						jObject.put("datetime",
								start.toString() + " " + hourDisplay + ":" + fromMinutes + ":" + fromSeconds);
						jObject.put("speed", row.getInt("max_speed"));
						jArray.put(jObject);
					}
				}
				...
				for (int i = 0; i < 365; i = i + 6) {
					...
					for (Row row : results) {
						...
						// jObject.put("time",jArray.length());
						jObject.put("datetime",
								start.plusDays(i).toString() + " " + fromHours + ":" + fromMinutes + ":" + fromSeconds);
						jObject.put("speed", row.getInt("max_speed"));
						jArray.put(jObject);
					}
				}
